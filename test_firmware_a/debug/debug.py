...
def set_log_enabled(name, enabled=True):
    ...
    Examples:
      set_log_enabled("sensor.co", False)           # Disable all CO sensor logs
      set_log_enabled("sensor.co.read", False)      # Disable only CO read logs
      set_log_enabled("communication.a", False)     # Disable all A communication logs
...
def is_log_enabled(name):
    ...
    Logic:
      1. Exact match: if "sensor.co.read" in flags, use that value
      2. Prefix match: if "sensor.co" in flags, use that (matches "sensor.co.read")
      3. Prefix match: if "sensor" in flags, use that (matches "sensor.co.read")
...
def log(name, message):
    """Simple logging function for test firmware with hierarchical per-channel flags."""
