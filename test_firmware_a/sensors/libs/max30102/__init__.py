...
from ustruct import unpack  # type: ignore
from utime import sleep_ms, ticks_diff, ticks_ms  # type: ignore
...
class MAX30102(object):
    ...
    def set_active_leds_amplitude(self, amplitude):
        if self._active_leds is not None and self._active_leds > 0:
            self.set_pulse_amplitude_red(amplitude)
        if self._active_leds is not None and self._active_leds > 1:
            self.set_pulse_amplitude_it(amplitude)
        if self._active_leds is not None and self._active_leds > 2:
    ...
    def update_acquisition_frequency(self):
        if None in [self._sample_rate, self._sample_avg]:
            ...
            if self._sample_rate is not None and self._sample_avg is not None:
    ...
    def check(self):
        ...
        if read_pointer != write_pointer:
            ...
            for i in range(number_of_samples):
                ...
                if self._multi_led_read_mode is None:
                    return False
